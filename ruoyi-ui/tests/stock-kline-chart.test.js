...
assert.deepStrictEqual(option.series.slice(1, 4).map(series => series.lineStyle.color), ['#000000', '#F5C400', '#E5484D'])
option.series.slice(1, 4).forEach(series => {
  assert.strictEqual(series.itemStyle.color, series.lineStyle.color)
  assert.strictEqual(series.emphasis.itemStyle.color, series.lineStyle.color)
  assert.strictEqual(series.emphasis.lineStyle.color, series.lineStyle.color)
})
...
const overviewPath = path.resolve(__dirname, '../src/components/StockAnalysisOverview/index.vue')
assert(fs.existsSync(overviewPath), 'StockAnalysisOverview component must exist')
const overviewSource = fs.readFileSync(overviewPath, 'utf8').replace(/\r\n/g, '\n')
...
const analyzerOverviewIndex = analyzerSource.indexOf('<stock-analysis-overview', resultTemplateIndex)
const overviewQuoteIndex = overviewSource.indexOf('class="stock-detail-panel stock-analysis-overview__quote"')
const overviewChartIndex = overviewSource.indexOf('<stock-kline-chart')
const overviewBasicsIndex = overviewSource.indexOf('class="stock-card stock-analysis-overview__basics"')

assert(resultTemplateIndex >= 0, 'analyzer must keep the successful-result template')
assert(analyzerOverviewIndex > resultTemplateIndex, 'analyzer must render the shared analysis overview')
assert(overviewQuoteIndex >= 0 && overviewQuoteIndex < overviewChartIndex, 'stock identity and price must render above the K-line chart')
assert(overviewBasicsIndex > overviewChartIndex, 'stock basics must render beside the K-line chart')
assert(
  /<stock-analysis-overview\s+:result="result"\s+:updated-at="formattedResultSavedAt"\s*\/>/s.test(analyzerSource),
  'analyzer must pass the result and formatted timestamp to the shared overview'
)
assert(
  analyzerSource.includes("import StockAnalysisOverview from '@/components/StockAnalysisOverview'"),
  'analyzer must import StockAnalysisOverview'
)
assert(/components:\s*{\s*StockAnalysisOverview[,\s]/s.test(analyzerSource), 'analyzer must register StockAnalysisOverview')
assert(/grid-template-columns:\s*repeat\(2, minmax\(0, 1fr\)\)/.test(overviewSource), 'overview must use an equal two-column desktop layout')
assert(/grid-template-columns:\s*repeat\(6, minmax\(0, 1fr\)\)/.test(overviewSource), 'overview must distribute all six basic metrics across the full row')
assert(overviewSource.includes('<strong>均线趋势</strong>'), 'overview right column must include the moving-average trend')
assert(overviewSource.includes('<strong>交易信号</strong>'), 'overview right column must include the trading signal')
assert(/@media \(min-width: 1201px\)[\s\S]*?\.stock-analysis-overview__side\s*{[\s\S]*?grid-template-rows:\s*repeat\(3, minmax\(0, 1fr\)\);[\s\S]*?align-self:\s*stretch;[\s\S]*?\.stock-analysis-overview__basics,[\s\S]*?\.stock-analysis-overview__strategy\s*{\s*height: 100%;\s*min-height: 0;/s.test(overviewSource), 'desktop overview side modules must evenly share the K-line chart height')
...
assert(/\.stock-kline-chart\s*{\s*display: flex;\s*height: 100%;\s*flex-direction: column;/s.test(componentSource), 'chart card must fill the overview grid height')
assert(/\.stock-kline-chart__canvas\s*{\s*flex: 1;\s*width: 100%;\s*min-height: 520px/s.test(componentSource), 'desktop chart must fill remaining card space with a 520px minimum height')
assert(/@media \(max-width: 768px\)[\s\S]*?\.stock-kline-chart__canvas\s*{\s*min-height: 420px/s.test(componentSource), 'mobile chart must retain a 420px minimum height under the narrow breakpoint')
assert(/\.stock-kline-chart__empty\s*{\s*display: flex;[\s\S]*?align-items: center;\s*justify-content: center;\s*min-height: 240px/s.test(componentSource), 'empty state must fill the card and remain centered with a 240px minimum height')
