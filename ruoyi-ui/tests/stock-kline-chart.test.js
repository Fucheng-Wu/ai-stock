...
assert(/watch:\s*{\s*klineData:\s*{\s*deep:\s*true,/s.test(componentSource), 'klineData must use a deep watcher')
assert(componentSource.includes('this.$nextTick(() => this.renderChart())'), 'initial and watched renders must wait for the DOM update')
...
assert(componentSource.includes("window.addEventListener('resize', this.handleResize)"), 'component must register a resize listener')
