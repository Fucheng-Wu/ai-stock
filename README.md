...
- **自选快照**：自选分析结果保存到数据库，再次展开时直接读取最近一次分析。
...
| 我的自选 | 关注股票、自动识别名称、展开分析 | 数据库保存最近分析快照 |
...
| GET | `/stock/watchlist/{watchlistId}/analysis` | 读取自选分析快照 |
| POST | `/stock/watchlist/{watchlistId}/analyze` | 重新分析自选并保存快照 |
