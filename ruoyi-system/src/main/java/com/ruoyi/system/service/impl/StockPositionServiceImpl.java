...
public class StockPositionServiceImpl implements IStockPositionService {
  ...
  public String marketForCode(String rawCode) { return StockCodeUtils.normalizeMarketCode(rawCode).substring(0, 2); }
  ...
  public void add(Long userId, StockPosition p, String name) { valid(p); String marketCode=StockCodeUtils.normalizeMarketCode(p.getStockCode());String code=marketCode.substring(2);p.setMarket(marketCode.substring(0,2));p.setStockCode(code);if (mapper.exists(userId, code)) throw new ServiceException("duplicate position"); p.setUserId(userId); p.setCreateBy(name); mapper.insert(p); }
